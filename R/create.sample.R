...
structure(function(# Function to create grain-size distributions
  ...
  data,
  ### Numeric matrix or data frame with two columns: depth (y-coordinates, in 
  ### metric units) and diameter of the particles (either metric units or 
  ### phi-scale).
  window.width,
  ### Numeric scalar, vertical window width. All particles whose y-coordinates
  ### fall into this moving window are sampled to build the grain-size 
  ### distribution for this window.
  step.size,
  ### Numeric scalar, amount of vertical distance between each sample, i.e.
  ### moving window mid point.
  class.limits
  ### Numeric vector, specifying the class limits of the grain-size ditribution
  ...
  list(sample = samples, ##<< Numeric matrix, sample grain-size distributions.
       n = n)            ##<< Numeric vector, number of speciment per sample.
  
  ##seealso<<
  ## \code{\link{interpolate.classes}}
  
  ##references<<
  ## Francus P. 1998. An image-analysis technique to measure
